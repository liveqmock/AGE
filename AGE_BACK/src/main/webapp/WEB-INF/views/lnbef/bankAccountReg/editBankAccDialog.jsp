...
<script>
	$(document).ready(function(){	
		...
		$("#accNo").formValidator({ onShow: "请填写账户号码", onFocus:"",onCorrect: "格式正确" }).inputValidator({min:1,onError:"账户号码不能为空"}).regexValidator({ regExp: "bankNo", dataType: "enum", onError: "请填写20位以内数字的账户号码" }).ajaxValidator({  
	        type : "post",  
	        cache : false, 
	        addidvalue: false,
	        async:false, 
	        url : "${ctx}/lnbef/bankAccountReg/checkAccNoUniqueEdit",  
	        datatype : "json",
	      	data:{
				accNo:$("#accNo").val(),
				id:$("#id").val(),
		...
		$("#saveBtn").click(function(){
			if($.formValidator.pageIsValid()){
				Dialog.confirm('您确认保存交吗？',function(){
					 $.ajax( {
						url : "${ctx}/lnbef/bankAccountReg/editBankAcc?auditStatus=0"+GetUrlConNum(),
						dataType : "json",
						type : "post", 
						data:$("#form").serialize(),
						success : function(data) {
		...
		$("#submitBtn").click(function(){
			if($.formValidator.pageIsValid()){
				Dialog.confirm('您确认要提交吗？提交之后就不能修改了！',function(){
					 $.ajax( {
						url : "${ctx}/lnbef/bankAccountReg/editBankAcc?auditStatus=1"+GetUrlConNum(),
						dataType : "json",
						type : "post", 
						data:$("#form").serialize(),
						success : function(data) {
...
<input id="baseAccFlag" name="baseAccFlag" type="hidden" value="${clmsLoanBank.baseAccFlag}" />
<input id="clmsUserid" name="clmsUserid" type="hidden" value="${clmsUserid}" />
